...
void solver(int n, int m, const char *a, const char *b, int &d)
{
    int* ans_curr_i = new int[m+1];
    int* ans_prev_i = new int[m+1];
    for(int j=0; j<m+1;j++){
        ans_prev_i[j]=j;
    }
    int curr_i=1;
    while(curr_i < n+1){
        ans_curr_i[0]=curr_i;
        for (int j=1;j<m+1;j++){
            ans_curr_i[j]=std::min(ans_curr_i[j-1], ans_prev_i[j])+1;
            if (a[curr_i-1]==b[j-1]){
                ans_curr_i[j]=std::min(ans_prev_i[j-1], ans_curr_i[j]);
            } else{
                ans_curr_i[j]=std::min(ans_prev_i[j-1]+1, ans_curr_i[j]);
            }
        }
        curr_i++;
        for(int j=0; j<m+1;j++){
            ans_prev_i[j]=ans_curr_i[j];
        }
    }
    d=ans_prev_i[m];
    delete [] ans_curr_i;
